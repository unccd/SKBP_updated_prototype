...
unccdApp.service('resultsService', function () {
    ...
    this.buildFacets   = function (unformattedFacets, unformattedRangeFacet, searchParams) {
        ...
        for(var facetName in unformattedFacets){
            if(unformattedFacets[facetName].length > 0) {
                facets[facetName] = this.buildFacet(unformattedFacets[facetName], searchParams[facetName]);
            }
        }
    ...
    this.buildFacet = function (listOfFacetElements, searchParam) {
        var refinerEnabled;
        ...
        if(paramName && paramValues && (paramValues != '')){
            ...
            if(paramName === 'fq'){ //Query
                ...
                for(var i = 0; i < paramList.length; i++){
                    queryString = queryString + 'fq='+paramName+':'+this.decodeRefiner(paramList[i]);

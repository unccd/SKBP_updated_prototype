...
set JAVA_PROPS_PRUNSRV=-Xrs#-Dfelix.config.properties=file:config/felix.properties#-Dcom.searchtechnologies.aspire.home=%ASPIRE_HOME%#-XX:PermSize=256m#-Xms2048m#-Xmx8192m#-XX:MaxPermSize=512#-XX:+ParallelRefProcEnabled#-XX:+AggressiveOpts#-XX:+CMSClassUnloadingEnabled#-XX:+UseConcMarkSweepGC
...
"%PRUNSRV_PATH%" //IS//%ASPIRE_SERVICE_NAME% --Install="%PRUNSRV_PATH%" --DisplayName="%ASPIRE_SERVICE_NAME%" --Startup=manual --StartPath="%ASPIRE_HOME%" --JavaHome="%JAVA_HOME%" --LibraryPath="%JAVA_HOME%\bin" --Jvm="%JAVA_JVM%" --JvmOptions="%JAVA_PROPS_PRUNSRV%" --Classpath="%ASPIRE_HOME%\bin\felix.jar" --StartMode=jvm --StartClass=org.apache.felix.main.Main --StopMode=exe --StopImage="%ASPIRE_HOME%\bin\shutdown.exe" --StopParams="%ASPIRE_HOME%\config\felix.properties" --StopTimeout=30 --LogPath="%ASPIRE_LOG%" --LogPrefix=%PROCRUN_LOGFILE_PREFIX% --StdOutput="%ASPIRE_LOG%\%ASPIRE_SERVICE_NAME%-stdout.log" --StdError="%ASPIRE_LOG%\%ASPIRE_SERVICE_NAME%-stderr.log"

...
unccdApp.service('resultsService', ['$sce', function ($sce) {
    ...
    this.buildFacets = function (unformattedFacets, unformattedRangeFacet, searchParams) {
      ...
      for (var facetName in unformattedRangeFacet) {
        if (Object.getOwnPropertyNames(unformattedRangeFacet[facetName]).length > 0) {
          facets[facetName] = this.buildRangeFacet(unformattedRangeFacet[facetName], searchParams[facetName], facetName);
        }
      }
    ...
    this.buildRangeFacet = function (rangeFacetContent, searchParam, facetName) {
      console.log(rangeFacetContent, searchParam);
      var rangeElements = rangeFacetContent.counts;

      var formattedFacets = [];
      var used = this.isDateParam(searchParam);
      ...
      if (rangeFacetContent.before > 0) {
        ...
        if (used) {
          this.usedFilters.push({"field": facetName, "value": "Before " + rangeFacetContent.start});
        }
      }
      //END - BEFORE
      ...
      for (var i = 0; i < rangeElements.length; i = i + 2) {
        if (rangeElements[i + 2]) {
          ...
          if (used) {
            this.usedFilters.push({"field": facetName,
              "value": "From " + rangeElements[i] + ' to ' + rangeElements[i + 2]});
          }
        } else {
          ...
          if (used) {
            this.usedFilters.push({"field": facetName, "value": "From " + rangeElements[i] + ' to ' + (parseInt(rangeElements[i]) + rangeFacetContent.gap)});
          }
        }
      }
      //END - BETWEEN
      ...
      if (rangeFacetContent.after > 0) {
        ...
        if (used) {
          this.usedFilters.push({"field": facetName, "value": "After " + rangeFacetContent.end});
        }
      }
      //END - AFTER
